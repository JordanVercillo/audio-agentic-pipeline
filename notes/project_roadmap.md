...
| W-3 | **Phase-4 webapp missing** while docs claim it ✅ | 🟠 Med | Upload lost it (or it never left the Copilot copy) | **DECIDED 2026-07-03 (SPEC D-7):** rebuild as P8 production pilot — per-visitor PKCE auth (no secret), feature-store joins, RAG insights. **DESIGN APPROVED 2026-07-05: FastAPI + Jinja2; build plan in [`docs/P8_PLAN.md`](../docs/P8_PLAN.md); building in 4 slices (auth+dash → RAG → container → Cloud Run).** |
...
| W-5 | ✅ ~~Env not reproducible~~ | ~~🟡~~ | Pre-uv era | **RESOLVED (P6):** `pyproject.toml` + `uv.lock` pin the full graph; `requirements.txt` kept as a pip export. `faiss-cpu` dropped 2026-07-31 with the unused vector stack (`docs/DELETIONS.md`). Note: plain `uv sync` PRUNES the `dev` extra — use `uv sync --extra dev` or pytest/ruff vanish. |
| W-6 | ✅ ~~No CI~~ | ~~🟡~~ | Never set up | **RESOLVED:** GitHub Actions runs ruff + the full suite on every push/PR, plus gitleaks and a coverage report. Green on every commit of session 72. |
